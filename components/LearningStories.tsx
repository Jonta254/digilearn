...
export function LearningStories() {
  return <section className="home-section learning-stories" aria-labelledby="stories-title"><div className="section-heading"><div><p className="eyebrow">Learning in practice</p><h2 id="stories-title">Skills become credible when the work can be reviewed.</h2><p className="section-intro">Real professional settings connect tools with evidence, feedback and responsibility.</p></div></div><div className="story-grid">{stories.map((story, index) => { const image=COURSE_IMAGE_ATTRIBUTIONS.find((item)=>item.id===story.id)!; return <article className={index === 0 ? "lead-story" : undefined} key={story.id}><figure><Image src={image.file} alt={image.alt} fill sizes={index === 0 ? "(max-width: 800px) 100vw, 66vw" : "(max-width: 800px) 100vw, 33vw"}/><figcaption>Photo: <a href={image.sourceUrl} target="_blank" rel="noopener noreferrer">{image.creator} / {image.source}</a></figcaption></figure><div><span>{story.label}</span><h3>{story.title}</h3><p>{story.body}</p></div></article>; })}</div></section>;
}
