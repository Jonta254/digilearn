...
export function CourseCover({ course }: { course: Course }) { const e=editorialFor(course); return <div className={`course-cover cover-${e.family}`} role="img" aria-label={`${course.title}: ${e.project}`}><span className="cover-grid"/><span className="cover-panel"><i/><i/><i/></span><small>{course.topic.replace("-", " ")}</small></div>; }
