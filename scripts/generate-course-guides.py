...
def build(item):
    ...
    story=[Spacer(1,13*mm),p("PRACTICAL COURSE WORKBOOK","eye"),p(c["title"],"title"),p(e["outcome"],"sub"),info([("LEVEL",c["level"]),("GUIDED TIME",f"{round(u['durationMinutes']/60)} hours across 12 lessons"),("PROJECT",e["project"]),("SKILLS",", ".join(u["skills"][:6])),("EDITORIAL STATUS",f"{e['editorialStatus']} v{e['version']}")]),Spacer(1,7*mm),p("WHAT YOU WILL BE ABLE TO DO","h2"),listing(u["outcomes"]),p("WHO THIS IS FOR","h2"),p(u["intendedLearner"]),p("BEFORE YOU BEGIN","h2"),listing(u["prerequisites"]),Spacer(1,4*mm),HRFlowable(width="100%",color=LINE,thickness=.7),Spacer(1,3*mm),p("This open workbook is a structured draft undergoing course-specific editorial review. It does not provide certification.","small"),PageBreak(),
